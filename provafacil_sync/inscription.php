...
function local_provafacil_get_key($url, $legacy_key, $apitoken) {
 ...
 /*   $response = $curl->get($url . '?legacy_key=' . $legacy_key, [
        'CURLOPT_HTTPHEADER' => [
            'Authorization: Token ' . $apitoken,
            'Content-Type: application/json',
        ],
...
$PAGE->set_url('/local/provafacil_sync/inscription.php');
...
if ($courseid) {
    ...
    echo '<form method="post" action="confirmation.php">';  // Modificar o action para apontar para a página de confirmação
...
/**if (isset($_POST['enroll']) && !empty($_POST['students'])) {
    ...
    foreach ($students as $userid) {
        ...
        // Obter candidate key usando legacy_key.
        ...
        // Obter academic key (código da disciplina).
        ...
        if ($candidate_key && $academic_key) {
            $response = local_provafacil_enroll_student($candidate_key, $academic_key, $apitoken);
            if (isset($response['success'])) {
                echo "<p>Estudante {$user->firstname} inscrito com sucesso!</p>";
            } else {
                echo "<p>Erro ao inscrever o estudante {$user->firstname}.</p>";
            }
        } else {
            echo "<p>Erro ao obter dados para o estudante {$user->firstname}.</p>";
        }
    }
}**/
